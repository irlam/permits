        ...
        <?php if (!empty($sectionFields) && is_array($sectionFields)): ?>
          <div class="meta" style="padding-top:12px;">
            <?php foreach ($sectionFields as $fIdx => $field):
                ...
                $defaultValues = is_array($fieldDefault)
                    ? array_values(array_map('strval', $fieldDefault))
                    : [(string)$fieldDefault];
            ...
            ?>
              <div class="field">
                ...
                <?php if ($fieldType === 'textarea'): ?>
                  <textarea name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?> placeholder="<?=htmlspecialchars($fieldPlaceholder)?>"><?=htmlspecialchars($defaultValues[0] ?? '')?></textarea>
                <?php elseif (in_array($fieldType, ['select', 'multiselect'], true) && !empty($fieldOptions)): ?>
                  <select name="<?=$inputName?>" <?= $isMultiple ? 'multiple' : '' ?> <?= $fieldRequired ? 'required' : '' ?>>
                    ...
                    <?php foreach ($fieldOptions as $option): ?>
                      <?php $isSelected = in_array($option['value'], $defaultValues, true); ?>
                ...
                <?php elseif (in_array($fieldType, ['radio', 'checkbox'], true) && !empty($fieldOptions)): ?>
                  <div style="display:flex;flex-direction:column;gap:6px;">
                    <?php foreach ($fieldOptions as $oIdx => $option):
                        ...
                        $isChecked = in_array($option['value'], $defaultValues, true);
                    ?>
                ...
                <?php elseif (in_array($fieldType, ['date', 'time', 'datetime', 'datetime-local'], true)): ?>
                  ...
                  <input type="<?=$inputType?>" name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?> value="<?=htmlspecialchars((string)($defaultValues[0] ?? ''))?>" placeholder="<?=htmlspecialchars($fieldPlaceholder)?>">
                <?php elseif ($fieldType === 'number'): ?>
                  <input type="number" name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?> value="<?=htmlspecialchars((string)($defaultValues[0] ?? ''))?>" placeholder="<?=htmlspecialchars($fieldPlaceholder)?>"<?php
                ...
                <?php elseif (in_array($fieldType, ['email', 'tel', 'url'], true)): ?>
                  <input type="<?=$fieldType?>" name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?> value="<?=htmlspecialchars((string)($defaultValues[0] ?? ''))?>" placeholder="<?=htmlspecialchars($fieldPlaceholder)?>">
                <?php elseif ($fieldType === 'boolean'): ?>
                  <select name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?>>
                    <option value="">Select...</option>
                    <option value="yes" <?= in_array('yes', $defaultValues, true) ? 'selected' : '' ?>>Yes</option>
                    <option value="no" <?= in_array('no', $defaultValues, true) ? 'selected' : '' ?>>No</option>
                  </select>
                <?php else: ?>
                  <input type="text" name="<?=$inputName?>" <?= $fieldRequired ? 'required' : '' ?> value="<?=htmlspecialchars((string)($defaultValues[0] ?? ''))?>" placeholder="<?=htmlspecialchars($fieldPlaceholder)?>">
                <?php endif; ?>

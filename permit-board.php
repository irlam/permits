    ...
    $stmt = $db->pdo->prepare("
        ...
        ORDER BY
            CASE LOWER(f.status)
                ...
                WHEN 'awaiting_acceptance' THEN 2
                ELSE 3
            END,
...
function board_location(array $row): string
{
    ...
    if ($siteBlock !== '') {
        return mb_substr($siteBlock, 0, 150, 'UTF-8');
    }
...
function board_bucket(string $status): string
{
...
$groups = ['suspended' => [], 'pending' => [], 'acceptance' => [], 'active' => []];
foreach ($rows as $row) {
    $groups[board_bucket((string)($row['status'] ?? ''))][] = $row;
}
...
<style>
body{margin:0;background:#0f172a;color:#e5e7eb;font-family:system-ui,-apple-system,'Segoe UI',sans-serif}.shell{max-width:1500px;margin:0 auto;padding:24px 18px 80px}.top{display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:20px}.top h1{margin:3px 0 5px}.muted{color:#94a3b8}.actions{display:flex;gap:10px;flex-wrap:wrap}.btn{display:inline-flex;padding:10px 14px;border-radius:10px;text-decoration:none;font-weight:700;background:#1e293b;color:#e2e8f0;border:1px solid #475569}.btn.primary{background:var(--brand-primary);color:var(--brand-on-primary);border-color:transparent}.stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-bottom:18px}.stat{background:#111827;border:1px solid #334155;border-radius:15px;padding:15px}.stat strong{display:block;font-size:30px}.stat.suspended{border-color:#ef4444;background:#351313}.stat.acceptance{border-color:#f59e0b}.tools{display:flex;gap:10px;align-items:center;margin:0 0 20px;flex-wrap:wrap}.search{flex:1 1 300px;background:#111827;color:#f8fafc;border:1px solid #475569;border-radius:11px;padding:12px 14px;font-size:16px}.board{display:grid;grid-template-columns:repeat(4,minmax(260px,1fr));gap:14px;align-items:start}.lane{background:#111827;border:1px solid #334155;border-radius:17px;overflow:hidden}.lane__head{padding:15px 16px;border-bottom:1px solid #334155}.lane__head h2{font-size:18px;margin:0}.lane__head p{margin:5px 0 0;font-size:13px;color:#94a3b8}.lane.suspended{border-color:#991b1b}.lane.suspended .lane__head{background:#450a0a}.lane.acceptance .lane__head{background:#422006}.cards{padding:12px;display:grid;gap:11px}.card{background:#0b1220;border:1px solid #293548;border-radius:13px;padding:14px;display:grid;gap:9px}.card[data-hidden="true"]{display:none}.card.suspended{border-color:#ef4444}.card__top{display:flex;justify-content:space-between;gap:8px;align-items:flex-start}.card h3{font-size:16px;margin:0}.ref{font-size:12px;color:#94a3b8;margin-top:3px}.badge{font-size:11px;border-radius:999px;padding:5px 8px;font-weight:800;white-space:nowrap;background:#334155}.badge.red{background:#7f1d1d;color:#fecaca}.badge.amber{background:#713f12;color:#fde68a}.badge.green{background:#14532d;color:#bbf7d0}.detail{font-size:13px;color:#cbd5e1}.detail strong{color:#f8fafc}.flags{display:flex;gap:6px;flex-wrap:wrap}.flag{font-size:11px;padding:4px 7px;border-radius:7px;background:#1e293b;color:#cbd5e1}.flag.conflict{background:#7f1d1d;color:#fecaca}.card__actions{display:flex;gap:7px;flex-wrap:wrap}.card__actions a{font-size:12px;padding:7px 9px;border-radius:8px;text-decoration:none;background:#1e293b;color:#e2e8f0;border:1px solid #475569}.card__actions a.workflow{background:var(--brand-primary);color:var(--brand-on-primary);border-color:transparent}.empty{padding:16px;color:#64748b;text-align:center;font-size:13px}@media(max-width:1200px){.board{grid-template-columns:repeat(2,minmax(280px,1fr))}}@media(max-width:700px){.stats{grid-template-columns:repeat(2,minmax(0,1fr))}.board{grid-template-columns:1fr}.actions,.actions .btn{width:100%}.btn{justify-content:center}.shell{padding:16px 12px 60px}}
</style>
</head>
<body>
<div class="shell">
<header class="top">
<div><div class="muted"><?= htmlspecialchars($companyName, ENT_QUOTES, 'UTF-8') ?></div><h1>Operational Permit Board</h1><div class="muted"><?= $canViewAll ? 'System-wide live permit control' : 'Your current permits' ?> · refresh the page for the latest lifecycle state</div></div>
...
<?php
$laneMeta = [
 'suspended' => ['title'=>'⛔ Suspended','help'=>'Work stopped. Revalidate before resuming.','badge'=>'red'],
...
?>
<article class="card <?= $key === 'suspended' ? 'suspended' : '' ?>" data-search="<?= htmlspecialchars($searchText, ENT_QUOTES, 'UTF-8') ?>">
<div class="card__top"><div><h3><?= htmlspecialchars((string)$permit['template_name'], ENT_QUOTES, 'UTF-8') ?></h3><div class="ref">#<?= htmlspecialchars((string)$permit['ref_number'], ENT_QUOTES, 'UTF-8') ?></div></div><span class="badge <?= htmlspecialchars($meta['badge'], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(ucfirst(str_replace('_',' ',(string)$permit['status'])), ENT_QUOTES, 'UTF-8') ?></span></div>
...
<?php if (!empty($permit['valid_to'])): ?><div class="detail"><strong>Valid until:</strong> <?= htmlspecialchars(board_date((string)$permit['valid_to']), ENT_QUOTES, 'UTF-8') ?></div><?php endif; ?>
...
<div class="card__actions"><?php if (!empty($permit['unique_link'])): ?><a href="<?= htmlspecialchars($app->url('/view-permit-public.php?link=' . rawurlencode((string)$permit['unique_link'])), ENT_QUOTES, 'UTF-8') ?>">View</a><a class="workflow" href="<?= htmlspecialchars($app->url('/permit-workflow.php?link=' . rawurlencode((string)$permit['unique_link'])), ENT_QUOTES, 'UTF-8') ?>">Control / handover</a><?php endif; ?></div>
</article>
<?php endforeach; ?>
</div></section>
<?php endforeach; ?>

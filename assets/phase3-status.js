(() => {
  ...
  if (lead) {
    lead.textContent = 'See pending, awaiting-acceptance, active and suspended permits without needing a permit number.';
  }

  const style = document.createElement('style');
  style.textContent = `
    .live-permit-board { display:grid; gap:18px; }
    .live-permit-board__stats { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; }
    ...
    .live-permit-stat--suspended { border-color:rgba(248,113,113,.45); background:rgba(127,29,29,.18); }
    ...
    .live-permit-card--suspended { border:1px solid rgba(248,113,113,.48); border-left:5px solid #ef4444; background:rgba(69,10,10,.72); }
    ...
    .live-permit-card--suspended .live-permit-card__note { color:#fecaca; font-weight:700; }
  ...
  function render() {
    ...
    if (!list) return;
    if (visible.length === 0) {
      list.innerHTML = `<div class="live-permit-empty">${term ? 'No current permits match that search.' : 'There are no current permits at the moment.'}</div>`;
      return;
    }

    list.innerHTML = visible.map((permit) => {
      const state = ['pending', 'active', 'suspended'].includes(permit.status) ? permit.status : 'pending';
      ...
      const validTo = permit.valid_to ? `<span><strong>Valid until:</strong> ${escapeHtml(formatDate(permit.valid_to))}</span>` : '';
      const badgeClass = suspended ? 'status-badge--danger' : (pending ? 'status-badge--warning' : 'status-badge--success');

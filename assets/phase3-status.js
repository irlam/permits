(() => {
  ...
  if (lead) {
    lead.textContent = 'See permits awaiting approval and permits currently active. No permit number is needed.';
  }

  const style = document.createElement('style');
  style.textContent = `
    .live-permit-board { display:grid; gap:18px; }
    .live-permit-board__stats {
      display:grid;
    ...
    .permit-specific-lookup {
      border-top:1px solid rgba(148,163,184,.18);
      padding-top:18px;
    }
    .permit-specific-lookup > summary {
      cursor:pointer;
      font-weight:700;
      color:#e2e8f0;
      list-style:none;
    ...
    @media (max-width:600px) {
      .live-permit-board__stats { grid-template-columns:1fr 1fr; }
  ...
  board.innerHTML = `
    <div class="live-permit-board__stats" aria-label="Current permit totals">
      <article class="live-permit-stat">
        ...
        <span class="live-permit-stat__label">Pending approval</span>
      </article>
      <article class="live-permit-stat">
  ...
  if (header) {
    ...
    section.insertAdjacentElement('afterbegin', board);
  }

  // Keep the old exact-reference lookup as a fallback, but make the live board
  // the primary on-site experience.
  if (form) {
    ...
    hint.textContent = 'If the permit is not current, you can still use the email address and reference from the permit email.';
    ...
    // Server-rendered lookup results belong with the fallback lookup.
  ...
  function render() {
    ...
    const visible = permits.filter((permit) => {
      if (!term) return true;
      return [permit.permit_type, permit.reference, permit.location, permit.status_label]
        .some((value) => String(value || '').toLowerCase().includes(term));
    });

    if (!list) return;
    if (visible.length === 0) {
      list.innerHTML = `<div class="live-permit-empty">${term ? 'No current permits match that search.' : 'There are no pending or active permits at the moment.'}</div>`;
      return;
    }
    ...
    list.innerHTML = visible.map((permit) => {
      ...
      const location = permit.location
        ? `<span><strong>Area:</strong> ${escapeHtml(permit.location)}</span>`
        : '';
      const submitted = permit.submitted_at
        ? `<span><strong>Submitted:</strong> ${escapeHtml(formatDate(permit.submitted_at))}</span>`
        : '';
      const validTo = !pending && permit.valid_to
        ? `<span><strong>Valid until:</strong> ${escapeHtml(formatDate(permit.valid_to))}</span>`
        : '';

      return `
        <article class="live-permit-card live-permit-card--${pending ? 'pending' : 'active'}">
          <div class="live-permit-card__top">
            <div>
              <h3 class="live-permit-card__title">${escapeHtml(permit.permit_type || 'Permit')}</h3>
              <div class="live-permit-card__ref">Ref #${escapeHtml(permit.reference || '—')}</div>
            </div>
            <span class="status-badge ${pending ? 'status-badge--warning' : 'status-badge--success'}">${escapeHtml(permit.status_label || (pending ? 'Pending Approval' : 'Active'))}</span>
          </div>
          <div class="live-permit-card__meta">${location}${submitted}${validTo}</div>
          <p class="live-permit-card__note">${pending ? 'Awaiting manager approval.' : 'Approved and currently live on site.'}</p>
        </article>
      `;
    }).join('');
  }
  ...
  async function loadPermits() {
    ...
    try {
      const endpoint = new URL('api/current-permits.php', window.location.href);
      endpoint.hash = '';
      ...
      if (!payload || payload.success !== true || !Array.isArray(payload.permits)) {
      ...
      if (updated) {
        updated.textContent = `Last updated ${new Intl.DateTimeFormat('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(new Date())}`;
      }
      render();
    } catch (error) {
      ...
      if (refresh) {
        ...
        refresh.textContent = 'Refresh permits';
      }
    }
  }
  ...
  loadPermits();
  window.setInterval(() => {
    if (!document.hidden) loadPermits();
  }, 30000);
})();

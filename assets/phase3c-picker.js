...
(() => {
  ...
  function isInspectionTile(tile) {
    const name = tile.querySelector('.template-tile__name')?.textContent || '';
    return inspectionNames.includes(normalise(name));
  }
  ...
  function rewriteInspectionHref(tile) {
    ...
    const basePath = (() => {
      try {
        ...
        const marker = '/create-permit-public.php';
        const markerInspection = '/create-inspection-public.php';
        if (pathname.endsWith(marker)) return pathname.slice(0, -marker.length);
        if (pathname.endsWith(markerInspection)) return pathname.slice(0, -markerInspection.length);
      } catch {
        // fall through
      }
      return '';
  ...
  function addStyles() {
    ...
    style.textContent = `
      #inspections .template-tile {
        border-color: rgba(56, 189, 248, 0.24);
      }
      #inspections .template-tile:hover,
      #inspections .template-tile:focus-visible {
      ...
      .phase3c-inspection-modal-group {
        display: grid;
        gap: 12px;
        padding-top: 18px;
        margin-top: 8px;
      ...
      .phase3c-inspection-modal-group__header {
        display: grid;
        gap: 4px;
      }
      .phase3c-inspection-modal-group__title {
        margin: 0;
        color: #e0f2fe;
        font-size: 17px;
      }
      .phase3c-inspection-modal-group__lead {
        margin: 0;
        color: rgba(186, 230, 253, 0.76);
        font-size: 13px;
        line-height: 1.45;
      }
      .phase3c-inspection-modal-grid {
        display: grid;
        gap: 12px;
      }
    `;
  ...
  function separateHomepageTiles() {
    ...
    section.innerHTML = `
      <header class="section__header">
        ...
        <p class="section__lead">Complete site, building and final inspections without raising a permit-to-work. Significant findings can still be escalated and linked to the correct permit where needed.</p>
      </header>
  ...
  function scheduleEnhancement() {
    // Phase 2 groups the tiles on DOMContentLoaded. Run immediately after those
    // listeners have completed so Phase 3C can split inspections cleanly.
